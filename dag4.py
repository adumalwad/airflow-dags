...
with DAG("anup_dag_3", start_date=datetime(2022, 1, 1),
    schedule_interval="@daily", catchup=False) as dag:
        ...
        Total_Sale_B = PythonOperator(
            task_id="sum_b",
            python_callable=sum_B
        )
        Total_Sale_C = PythonOperator(
            task_id="sum_c",
            python_callable=sum_C
        )
        Total_Sale_D = PythonOperator(
            task_id="sum_d",
            python_callable=sum_D
        )
        ...
        [Company_A,Company_B,Company_C,Company_D] >> [Total_Sale_A,Total_Sale_B,Total_Sale_C,Total_Sale_D] >> Max_Month >> Best_Company >> [best_A,best_B,best_C,best_D]
